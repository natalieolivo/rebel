...
export class PageHead extends HTMLElement {
    public constructor() {
        ...
        const data = {
            navLinks: JSON.parse(<string>this.dataset.navLinks)
        };
        const html: string = this.getTemplate(data);
    ...
    private getTemplate(data: any): string {
        ...
        const templateLinks = links.map((link) => {
            ...
            <li class="verticals-flex-grow verticals-listitem" data-vertical-label="tech">
                <a href="${link.url}" class="verticals-listitem-link" data-e2e-name="vertical-listitem-link" data-vertical-listitem="Tech">
                    <span class="verticals-listitem-label">${link.label}</span>
                </a>
            </li>`;
        }).join(' ');
